...
`C:\Program Files\StataNow19\docs`

Useful manuals in this installation:

- Data Management Reference Manual: `C:\Program Files\StataNow19\docs\d.pdf`
- User's Guide: `C:\Program Files\StataNow19\docs\u.pdf`
- Base Reference Manual: `C:\Program Files\StataNow19\docs\r.pdf`
- Getting Started with Stata for Windows: `C:\Program Files\StataNow19\docs\gsw.pdf`
...
- `C:\Program Files\StataNow19\ado\base\d\do.sthlp`
- `C:\Program Files\StataNow19\ado\base\d\doedit.sthlp`
- `C:\Program Files\StataNow19\ado\base\c\contents_utilities_basic.sthlp`
...
Potentially useful tools in this environment:

- Python `pdfplumber`: installed in the active Python environment

`pdfgrep` is not installed natively on this Windows machine, so treat `rg` over extracted `.txt` or `.md` snippets as the practical substitute. If `pandoc`, `pdftotext`, or `pdfinfo` are needed, verify their locations with `Get-Command` before relying on them. At the time this skill was installed, Pandoc was not discoverable on `PATH`.
...
2. If the layout is simple and `pdftotext` is available, extract only those pages with it.
3. If Pandoc is available, convert the extracted snippet into Markdown when Markdown is easier to inspect or summarize.
...
pdf_path = Path(r"C:\Program Files\StataNow19\docs\d.pdf")
