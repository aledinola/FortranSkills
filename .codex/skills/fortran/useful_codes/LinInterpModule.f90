...
!   Check also Bewley toolkit of Fatih Guvenen's RA
!	-----------------------------------------------------------------

!Given to me (with permission) from Sun-Bin Kim
...
real(8) function lininterp2(x, y, xgrid, ygrid, ZZ)
	...
	t = (x - xgrid(xndx))/(xgrid(xndx+1) - xgrid(xndx))
	uu = (y - ygrid(yndx))/(ygrid(yndx+1) - ygrid(yndx))

	xyz = (1-t)*(1-uu)*ZZ(xndx,yndx) + t*(1-uu)*ZZ(xndx+1,yndx)	&
		+ t*uu*ZZ(xndx+1,yndx+1) + (1-t)*uu*ZZ(xndx,yndx+1)
